...
public class Transaction {
    ...
    public static UUID makeTransaction(User sender, User recipient, int transferAmount) throws IOException {
        if (transferAmount < 0) {
            throw new IllegalStateException("Transaction amount should be positive.");
        }
        if (sender.getBalance() - transferAmount < 0) {
            throw new IllegalStateException("Sender doesn't have enough money for this transaction.");
        }
        ...
        recipient.setCount(recipient.getCount() + 1);
    ...
    public void removeTransactionOnBothUsers(UUID uuid) throws IOException {
